    ...
    <div class="px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div class="overflow-hidden rounded-xl border border-gray-200 bg-white shadow-sm dark:border-gray-700 dark:bg-gray-800">
            <div class="border-b border-gray-200 px-6 py-4 dark:border-gray-700">
                ...
                <p class="text-sm text-gray-500 dark:text-gray-400">Modify the fields below. Leave file inputs empty to keep existing files.</p>
            </div>
            <div class="px-6 py-6">
                    ...
                    <div class="space-y-8">
                        ...
                        <div>
                            ...
                            <div class="mt-2 flex flex-wrap gap-6">
                                <label class="inline-flex cursor-pointer items-center rounded-lg border border-gray-300 px-4 py-2 transition-all has-[:checked]:border-primary has-[:checked]:bg-primary/5 dark:border-gray-600 dark:has-[:checked]:border-primary">
                                    <input type="radio" name="type" value="image" class="form-radio text-primary focus:ring-primary" {{ old('type', $gallery->type) == 'image' ? 'checked' : '' }}>
                                    <span class="ml-2 text-sm text-gray-700 dark:text-gray-300"><i class="bi bi-image mr-1"></i> Image / Before-After</span>
                                </label>
                                <label class="inline-flex cursor-pointer items-center rounded-lg border border-gray-300 px-4 py-2 transition-all has-[:checked]:border-primary has-[:checked]:bg-primary/5 dark:border-gray-600 dark:has-[:checked]:border-primary">
                                    <input type="radio" name="type" value="video" class="form-radio text-primary focus:ring-primary" {{ old('type', $gallery->type) == 'video' ? 'checked' : '' }}>
                                    <span class="ml-2 text-sm text-gray-700 dark:text-gray-300"><i class="bi bi-play-circle mr-1 text-primary"></i> Video / Embed</span>
                                </label>
                            </div>
                            @error('type') <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p> @enderror
                            <p class="mt-2 text-xs text-amber-600 dark:text-amber-400">
                                <i class="bi bi-exclamation-triangle mr-1"></i> Changing type will clear previously uploaded images or embed data.
                            </p>
                        </div>

                        {{-- Image Fields --}}
                        <div id="image-fields" class="{{ old('type', $gallery->type) == 'video' ? 'hidden' : '' }} space-y-6">
                            {{-- Main Image --}}
                            <div>
                                ...
                                @if($gallery->image)
                                    <div class="mt-2 mb-3" id="current-image-container">
                                        <img src="{{ asset($gallery->image) }}" class="h-20 w-20 rounded-lg object-cover shadow-sm">
                                        <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">Current image (leave empty to keep)</p>
                                    </div>
                                @endif
                                <div class="mt-1 flex items-center justify-center rounded-lg border-2 border-dashed border-gray-300 px-6 pt-5 pb-6 dark:border-gray-600">
                                    <div class="space-y-1 text-center">
                                        ...
                                        <div class="flex text-sm text-gray-600 dark:text-gray-400">
                                            <label for="image" class="relative cursor-pointer rounded-md font-medium text-primary hover:text-primary/80">
                                                <span>Upload new image</span>
                                                <input id="image" name="image" type="file" accept="image/*" class="sr-only">
                                            </label>
                                            <p class="pl-1">or drag and drop</p>
                                        </div>
                                        <p class="text-xs text-gray-500 dark:text-gray-400">JPG, PNG, WEBP up to 5MB (leave empty to keep current)</p>
                                    </div>
                                </div>
                                {{-- Preview for new image --}}
                                <div id="image-preview" class="mt-3 hidden">
                                    <img id="image-preview-img" class="h-20 w-20 rounded-lg object-cover shadow-sm">
                                    <button type="button" id="clear-image" class="text-xs text-red-600 mt-1">Remove new selection</button>
                                </div>
                            ...
                            {{-- Before Image --}}
                            <div>
                                <label for="before_image" class="block text-sm font-medium text-gray-700 dark:text-gray-300">Before Image (Optional)</label>
                                @if($gallery->before_image)
                                    <div class="mt-2 mb-2" id="current-before-container">
                                        <img src="{{ asset($gallery->before_image) }}" class="h-16 w-16 rounded object-cover shadow-sm">
                                        <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">Current</p>
                                    </div>
                                @endif
                                <div class="mt-1 flex items-center justify-center rounded-lg border-2 border-dashed border-gray-300 px-4 pt-4 pb-4 dark:border-gray-600">
                                    <div class="space-y-1 text-center">
                                        <i class="bi bi-camera text-2xl text-gray-400"></i>
                                        <div class="text-sm text-gray-600 dark:text-gray-400">
                                            <label for="before_image" class="relative cursor-pointer rounded-md font-medium text-primary hover:text-primary/80">
                                                <span>Choose new file</span>
                                                <input id="before_image" name="before_image" type="file" accept="image/*" class="sr-only">
                                            </label>
                                        </div>
                                    </div>
                                </div>
                                <div id="before-preview" class="mt-3 hidden">
                                    ...
                                    <button type="button" id="clear-before" class="text-xs text-red-600 mt-1">Remove new selection</button>
                                </div>
                                @error('before_image') <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p> @enderror
                            </div>

                            {{-- After Image --}}
                            <div>
                                <label for="after_image" class="block text-sm font-medium text-gray-700 dark:text-gray-300">After Image (Optional)</label>
                                @if($gallery->after_image)
                                    <div class="mt-2 mb-2" id="current-after-container">
                                        <img src="{{ asset($gallery->after_image) }}" class="h-16 w-16 rounded object-cover shadow-sm">
                                        <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">Current</p>
                                    </div>
                                @endif
                                <div class="mt-1 flex items-center justify-center rounded-lg border-2 border-dashed border-gray-300 px-4 pt-4 pb-4 dark:border-gray-600">
                                    <div class="space-y-1 text-center">
                                        <i class="bi bi-camera text-2xl text-gray-400"></i>
                                        <div class="text-sm text-gray-600 dark:text-gray-400">
                                            <label for="after_image" class="relative cursor-pointer rounded-md font-medium text-primary hover:text-primary/80">
                                                <span>Choose new file</span>
                                                <input id="after_image" name="after_image" type="file" accept="image/*" class="sr-only">
                                            </label>
                                        </div>
                                    </div>
                                </div>
                                <div id="after-preview" class="mt-3 hidden">
                                    ...
                                    <button type="button" id="clear-after" class="text-xs text-red-600 mt-1">Remove new selection</button>
                                </div>
                                @error('after_image') <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p> @enderror
                            </div>
                        </div>

                        {{-- Video Fields --}}
                        <div id="video-fields" class="{{ old('type', $gallery->type) == 'image' ? 'hidden' : '' }} space-y-6">
                            <div>
                                <label for="video_url" class="block text-sm font-medium text-gray-700 dark:text-gray-300">Video URL <span class="text-xs text-gray-500">(optional)</span></label>
                                ...
                                <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">Direct video file (.mp4/.webm) or platform URL (YouTube, Vimeo, etc.)</p>
                                @error('video_url') <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p> @enderror
                            </div>

                            <div>
                                <label for="embed_code" class="block text-sm font-medium text-gray-700 dark:text-gray-300">Embed Code <span class="text-xs text-gray-500">(optional)</span></label>
                                ...
                                <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">Paste embed code from Instagram, TikTok, Facebook, YouTube, etc. Leave empty if you use Video URL.</p>
                            </div>
                        </div>
