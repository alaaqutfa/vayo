...
@section('title', __t('Add Service'))

@section('content')
    <div class="px-4 sm:px-6 lg:px-8 py-8">
        <div class="mb-8 flex flex-wrap items-center justify-between gap-4">
            <div>
                <h1 class="text-3xl font-bold text-gray-900 dark:text-white">{{ __t('Add New Service') }}</h1>
                <p class="mt-1 text-sm text-gray-600 dark:text-gray-400">{{ __t('Create a new medical service offering') }}</p>
            </div>
            <a href="{{ route('admin.services.index') }}"
                ...
                <i class="bi bi-arrow-left mr-2"></i> {{ __t('Back to Services') }}
            </a>
        </div>
        ...
        <div
            ...
            <div class="border-b border-gray-200 px-6 py-4 dark:border-gray-700">
                <h2 class="text-xl font-semibold text-gray-800 dark:text-white">{{ __t('Service Information') }}</h2>
                <p class="text-sm text-gray-500 dark:text-gray-400">{{ __t('Fill in the details below to create a new service.') }}</p>
            </div>
            <div class="px-6 py-6">
                <form action="{{ route('admin.services.store') }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="grid grid-cols-1 gap-8 lg:grid-cols-2">
                        <div class="space-y-6">
                            <div>
                                <label for="category_id" class="block text-sm font-medium text-gray-700 dark:text-gray-300">{{ __t('Category') }}</label>
                                <select name="category_id" id="category_id"
                                    ...
                                    <option value="">{{ __t('No category') }}</option>
                            ...
                            <div>
                                <label for="name" class="block text-sm font-medium text-gray-700 dark:text-gray-300">{{ __t('Service Name') }} <span class="text-red-500">*</span></label>
                            ...
                            <div>
                                ...
                                <div class="relative mt-1">
                                    ...
                                    <i class="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400 {{ old('icon') }}"></i>
                                </div>
                                <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">{{ __t('Use Bootstrap Icons class (e.g., bi bi-briefcase)') }}</p>
                                @error('icon') <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                                @enderror
                            </div>

                            <div>
                                <label for="image"
                                    class="block text-sm font-medium text-gray-700 dark:text-gray-300">{{ __t('Service Image') }}</label>
                                <div
                                    ...
                                    <div class="space-y-1 text-center">
                                        ...
                                        <div class="flex text-sm text-gray-600 dark:text-gray-400">
                                            <label for="image"
                                                ...
                                                <span>{{ __t('Upload a file') }}</span>
                                                <input id="image" name="image" type="file" accept="image/*" class="sr-only">
                                            </label>
                                            <p class="pl-1">{{ __t('or drag and drop') }}</p>
                                        </div>
                                        <p class="text-xs text-gray-500 dark:text-gray-400">{{ __t('PNG, JPG, WEBP up to 2MB') }}</p>
                                    </div>
                                </div>
                            ...
                            <div>
                                <label for="order"
                                    class="block text-sm font-medium text-gray-700 dark:text-gray-300">{{ __t('Display Order') }}</label>
                            ...
                            <div class="flex items-center">
                                ...
                                <label for="is_active" class="ml-2 block text-sm text-gray-700 dark:text-gray-300">{{ __t('Active (visible on frontend)') }}</label>
                            </div>
                        </div>
                        ...
                        <div class="space-y-6">
                            <div>
                                <label for="description"
                                    class="block text-sm font-medium text-gray-700 dark:text-gray-300">{{ __t('Description') }} <span class="text-red-500">*</span></label>
                                <textarea name="description" id="description" rows="6"
                                    class="mt-1 block w-full rounded-lg border-gray-300 shadow-sm focus:border-primary focus:ring-primary dark:border-gray-600 dark:bg-gray-700 dark:text-white dark:placeholder-gray-400 sm:text-sm"
                                    required>{{ old('description') }}</textarea>
                                @error('description') <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}
                                </p> @enderror
                            </div>

                            <div>
                                <label class="block text-sm font-medium text-gray-700 dark:text-gray-300">{{ __t('Features (key points)') }}</label>
                                    ...
                                    @if(old('features'))
                                        @foreach(old('features') as $feature)
                                            <div class="feature-row flex gap-2">
                                                ...
                                                <button type="button"
                                                    class="remove-feature rounded-lg bg-red-100 px-3 py-2 text-sm text-red-700 transition-colors hover:bg-red-200 dark:bg-red-900/30 dark:text-red-300 dark:hover:bg-red-900/50">{{ __t('Remove') }}</button>
                                            </div>
                                        @endforeach
                                    @else
                                        <div class="feature-row flex gap-2">
                                            <input type="text" name="features[]" placeholder="{{ __t('e.g., 24/7 Emergency Cardiac Care') }}"
                                                class="flex-1 rounded-lg border-gray-300 shadow-sm focus:border-primary focus:ring-primary dark:border-gray-600 dark:bg-gray-700 dark:text-white sm:text-sm">
                                            <button type="button"
                                                class="remove-feature hidden rounded-lg bg-red-100 px-3 py-2 text-sm text-red-700 transition-colors hover:bg-red-200 dark:bg-red-900/30 dark:text-red-300 dark:hover:bg-red-900/50">{{ __t('Remove') }}</button>
                                        </div>
                                    @endif
                                </div>
                                <button type="button" id="add-feature"
                                    ...
                                    <i class="bi bi-plus-circle mr-1"></i> {{ __t('Add another feature') }}
                                </button>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="mt-8 flex justify-end border-t border-gray-200 pt-6 dark:border-gray-700">
                        <button type="submit"
                            ...
                            <i class="bi bi-check-lg mr-2 text-lg"></i> {{ __t('Create Service') }}
                        </button>
                    </div>
                </form>
            </div>
        ...
        <script>
            document.addEventListener('DOMContentLoaded', function () {
                ...
                addBtn.addEventListener('click', () => {
                    ...
                    newRow.innerHTML = `
                                <input type="text" name="features[]" placeholder="{{ __t('New feature') }}" class="flex-1 rounded-lg border-gray-300 shadow-sm focus:border-primary focus:ring-primary dark:border-gray-600 dark:bg-gray-700 dark:text-white sm:text-sm">
                                <button type="button" class="remove-feature rounded-lg bg-red-100 px-3 py-2 text-sm text-red-700 transition-colors hover:bg-red-200 dark:bg-red-900/30 dark:text-red-300 dark:hover:bg-red-900/50">{{ __t('Remove') }}</button>
                            `;
