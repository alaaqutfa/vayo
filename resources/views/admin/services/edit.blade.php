...
@section('title', __t('Edit Service'))

@section('content')
    <div class="px-4 sm:px-6 lg:px-8 py-8">
        <div class="mb-8 flex flex-wrap items-center justify-between gap-4">
            <div>
                <h1 class="text-3xl font-bold text-gray-900 dark:text-white">{{ __t('Edit Service') }}</h1>
                <p class="mt-1 text-sm text-gray-600 dark:text-gray-400">{{ __t('Update details for') }} "{{ $service->name }}"</p>
            </div>
        ...
        <div class="overflow-hidden rounded-xl border border-gray-200 bg-white shadow-sm dark:border-gray-700 dark:bg-gray-800">
            <div class="border-b border-gray-200 px-6 py-4 dark:border-gray-700">
                ...
                <p class="text-sm text-gray-500 dark:text-gray-400">{{ __t('Modify the fields below to update this service.') }}</p>
            </div>
            <div class="px-6 py-6">
                <form action="{{ route('admin.services.update', $service->id) }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="grid grid-cols-1 gap-8 lg:grid-cols-2">
                        ...
                        <div class="space-y-6">
                            <div>
                                <label for="description" class="block text-sm font-medium text-gray-700 dark:text-gray-300">{{ __t('Description') }} <span class="text-red-500">*</span></label>
                                        ...
                                        @empty
                                            <div class="feature-row flex gap-2">
                                                <input type="text" name="features[]" placeholder="{{ __t('e.g., 24/7 Emergency Cardiac Care') }}"
                                                       class="flex-1 rounded-lg border-gray-300 shadow-sm focus:border-primary focus:ring-primary dark:border-gray-600 dark:bg-gray-700 dark:text-white sm:text-sm">
                                                <button type="button" class="remove-feature hidden rounded-lg bg-red-100 px-3 py-2 text-sm text-red-700 transition-colors hover:bg-red-200 dark:bg-red-900/30 dark:text-red-300 dark:hover:bg-red-900/50">{{ __t('Remove') }}</button>
                                            </div>
                                        @endforelse
                                    @endif
                                </div>
                    ...
                    <div class="mt-8 flex justify-end border-t border-gray-200 pt-6 dark:border-gray-700">
                        <button type="submit"
                            ...
                            <i class="bi bi-save mr-2 text-lg"></i> {{ __t('Update Service') }}
                        </button>
                    </div>
                </form>
            </div>
