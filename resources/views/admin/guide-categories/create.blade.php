...
@section('title', __t('Add Guide Category'))

@section('content')
    <div class="px-4 sm:px-6 lg:px-8 py-8">
        <div class="mb-8 flex flex-wrap items-center justify-between gap-4">
            <div>
                <h1 class="text-3xl font-bold text-gray-900 dark:text-white">{{ __t('Add Guide Category') }}</h1>
                <p class="mt-1 text-sm text-gray-600 dark:text-gray-400">{{ __t('Create a new main or sub category for the guide.') }}</p>
            </div>
        ...
        <div class="overflow-hidden rounded-xl border border-gray-200 bg-white shadow-sm dark:border-gray-700 dark:bg-gray-800">
            ...
            <form action="{{ route('admin.guide-categories.store') }}" method="POST" class="px-6 py-6">
                @csrf
                ...
                <div class="mt-6">
                    ...
                    <textarea name="description" id="description" rows="5"
                        class="mt-1 block w-full rounded-lg border-gray-300 shadow-sm focus:border-primary focus:ring-primary dark:border-gray-600 dark:bg-gray-700 dark:text-white sm:text-sm">{{ old('description') }}</textarea>
                ...
                <div class="mt-8 flex justify-end border-t border-gray-200 pt-6 dark:border-gray-700">
                    <button type="submit"
                        ...
                        <i class="bi bi-check-lg mr-2"></i> {{ __t('Create Category') }}
                    </button>
                </div>
            </form>
        </div>
