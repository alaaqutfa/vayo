...
@section('content')
        ...
        <div class="mb-8 rounded-2xl bg-gradient-to-r from-primary/5 to-primary/10 p-6 backdrop-blur-sm dark:from-primary/20 dark:to-primary/5">
            <div class="sm:flex sm:items-center sm:justify-between">
                <div>
                    ...
                    <p class="mt-2 text-sm text-gray-600 dark:text-gray-300">Manage all text translations for the website.</p>
                </div>
                ...
                <div class="overflow-x-auto">
                    <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                        <thead class="bg-gray-50 dark:bg-gray-800/50">
                            <tr>
                                <th class="px-6 py-4 text-left text-xs font-medium uppercase tracking-wider text-gray-500 dark:text-gray-400">Key</th>
                                <th class="px-6 py-4 text-left text-xs font-medium uppercase tracking-wider text-gray-500 dark:text-gray-400">Value ({{ strtoupper($locale) }})</th>
                                <th class="px-6 py-4 text-right text-xs font-medium uppercase tracking-wider text-gray-500 dark:text-gray-400">Actions</th>
                            </tr>
                        </thead>
                        <tbody class="divide-y divide-gray-200 bg-white dark:divide-gray-700 dark:bg-gray-900">
                            @foreach($translations as $trans)
                                <tr class="transition-colors hover:bg-gray-50 dark:hover:bg-gray-800/50">
                                    <td class="whitespace-nowrap px-6 py-4 text-sm font-medium text-gray-900 dark:text-white">{{ $trans->key }}</td>
                                    <td class="px-6 py-4">
                                        <input type="text" name="translations[{{ $trans->key }}]"
                                               value="{{ $trans->value }}"
                                    ...
                                    <td class="whitespace-nowrap px-6 py-4 text-right text-sm font-medium">
                                        <form action="{{ route('admin.translations.destroy', $trans->id) }}" method="POST" class="inline" onsubmit="return confirm('Are you sure?');">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit" class="inline-flex items-center text-red-600 hover:text-red-800 dark:text-red-400 dark:hover:text-red-300">
    ...
    <div id="addKeyModal" class="fixed inset-0 z-50 hidden overflow-y-auto" aria-labelledby="modal-title" role="dialog" aria-modal="true">
        <div class="flex min-h-screen items-end justify-center px-4 pb-20 pt-4 text-center sm:block sm:p-0">
            ...
            {{-- Modal panel --}}
            <div class="inline-block transform overflow-hidden rounded-xl bg-white text-left align-bottom shadow-xl transition-all sm:my-8 sm:w-full sm:max-w-lg sm:align-middle dark:bg-gray-800">
                <form action="{{ route('admin.translations.key.store') }}" method="POST">
                    ...
                    <div class="bg-white px-6 pt-5 pb-4 dark:bg-gray-800 sm:p-6 sm:pb-4">
                        <div class="flex items-center justify-between pb-3">
                            <h3 class="text-lg font-semibold text-gray-900 dark:text-white" id="modal-title">Add New Translation Key</h3>
                        ...
                        <div class="mt-4">
                            <label for="key_input" class="block text-sm font-medium text-gray-700 dark:text-gray-300">Key <span class="text-red-500">*</span></label>
                            <input type="text" name="key" id="key_input" required
                                   placeholder="e.g., home.title, about.hero"
                            ...
                            <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">Use dot notation for grouping (e.g., home.welcome).</p>
                        </div>

                        <div class="mt-4">
                            <label for="value_input" class="block text-sm font-medium text-gray-700 dark:text-gray-300">Value ({{ strtoupper($locale) }})</label>
                    ...
                    <div class="bg-gray-50 px-6 py-3 dark:bg-gray-900 sm:flex sm:flex-row-reverse sm:px-6">
                        ...
                        <button type="button"
                                onclick="document.getElementById('addKeyModal').classList.add('hidden')"
